  ...
  try {
    ...
    const response = await fetch("https://api.nowpayments.io/v1/invoice", {
      ...
      body: JSON.stringify({
        ...
        pay_currency:      payCurrency || "usdtsol",
        order_id:          orderId,
    ...
    if (!response.ok) {
      console.log("NOWPayments error:", response.status, JSON.stringify(data));
      return Response.json({ error: data.message || "NOWPayments order failed" }, { status: 500 });
    }

    return Response.json({ url: data.invoice_url });
  } catch (error) {
    console.log("NOWPayments catch:", error.message, error.cause);
