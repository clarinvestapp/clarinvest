  ...
  return (
    <div onClick={() => onClick(s.ticker, s.type)}
      ...
      <button onClick={handleRemove} disabled={removing}
        ...
        style={{ position:"absolute", top:"0.9rem", right:"0.9rem", background:hov?c.surface:"transparent", border:`1px solid ${hov?c.borderHi:"transparent"}`, borderRadius:"4px", padding:"3px 7px", cursor:"pointer", color:c.muted, fontSize:"0.75rem", transition:"opacity 0.18s ease", opacity:removing?0.4:1 }}>
        {removing ? "…" : "✕"}
      </button>
      ...
      <div style={{ marginBottom:"0.85rem" }}>
        {hasPrice ? (
          <>
            <span style={{ fontFamily:gs, fontSize:"1.4rem", fontWeight:600, color:c.text }}>${s.price.toFixed(2)}</span>
            <span style={{ fontFamily:gs, fontSize:"0.84rem", fontWeight:600, color:pos?c.green:c.red, marginLeft:"0.5rem" }}>
...
function SkeletonCard({ c }) {
  return (
    <div style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:"16px", padding:"1.5rem 1.4rem" }}>
    ...
    <div style={{ background:c.bg, minHeight:"100vh", fontFamily:gs }}>
      ...
      <div style={{ maxWidth:"1200px", margin:"0 auto", padding:"2.5rem 3.5rem" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:"2rem", flexWrap:"wrap", gap:"1rem" }}>
          <div>
            <p style={{ fontFamily:gs, color:c.muted, fontSize:"0.65rem", letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"0.4rem", fontWeight:500 }}>Portfolio</p>
            <h1 style={{ fontFamily:gs, fontSize:"clamp(1.5rem,3vw,2.2rem)", fontWeight:600, color:c.text }}>
          ...
          <button onClick={() => router.push("/dashboard")}
            style={{ background:c.text, color:c.bg, border:"none", borderRadius:"4px", padding:"9px 20px", fontFamily:gs, fontSize:"0.82rem", fontWeight:600, cursor:"pointer" }}>
            + Add stocks
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:"center", padding:"6rem 2rem" }}>
            ...
            <h2 style={{ fontFamily:gs, fontSize:"1.3rem", fontWeight:600, color:c.text, marginBottom:"0.75rem" }}>Your watchlist is empty</h2>
            ...
            <button onClick={() => router.push("/dashboard")}
              style={{ background:c.text, color:c.bg, border:"none", borderRadius:"4px", padding:"11px 28px", fontFamily:gs, fontSize:"0.84rem", fontWeight:600, cursor:"pointer" }}>
              Go to Discovery →
            </button>
          </div>
        ) : (
