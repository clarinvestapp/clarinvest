...
export default function BannersPage() {
  ...
  // Always fetch a fresh token — avoids stale session issues
  ...
  const save = async (e) => {
    ...
    try {
      const headers = await authHeaders();
      if (editing) {
        const res = await fetch("/api/admin/banners", { method:"PATCH", headers, body:JSON.stringify({ id:editing, ...body }) });
        const data = await res.json();
        ...
        const res = await fetch("/api/admin/banners", { method:"POST", headers, body:JSON.stringify(body) });
        const data = await res.json();
  ...
  const toggleActive = async (banner) => {
    ...
    try {
      const headers = await authHeaders();
      const res = await fetch("/api/admin/banners", { method:"PATCH", headers, body:JSON.stringify({ id:banner.id, active:newVal }) });
      if (!res.ok) {
        // Revert on failure
  ...
  const move = async (idx, dir) => {
    ...
    try {
      const headers = await authHeaders();
      await Promise.all([
        fetch("/api/admin/banners", { method:"PATCH", headers, body:JSON.stringify({ id:next[idx].id, sort_order:idx }) }),
        fetch("/api/admin/banners", { method:"PATCH", headers, body:JSON.stringify({ id:next[swap].id, sort_order:swap }) }),
      ]);
  ...
  return (
    <div style={{ padding:"2rem 2.5rem", maxWidth:"900px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:"2rem", flexWrap:"wrap", gap:"1rem" }}>
        ...
        <button onClick={() => { if (showForm && !editing) { setShowForm(false); } else { setEditing(null); setForm(EMPTY); setShowForm(true); } }}
        ...
        <form onSubmit={save} style={{ background:c.card, border:"1px solid "+c.borderHi, borderRadius:"12px", padding:"1.5rem", marginBottom:"1.5rem" }}>
          ...
          <div style={{ marginBottom:"0.85rem" }}>
            ...
            <textarea value={form.text} onChange={e=>setForm(p=>({...p,text:e.target.value}))} rows={2}
              placeholder="e.g. 🎉 20% off all annual plans — use code LAUNCH20"
          ...
          {/* Live preview */}
          {form.text && (
            <div style={{ background:(TYPE_COLORS[form.type]?.col(c)||c.blue)+"1A", border:"1px solid "+(TYPE_COLORS[form.type]?.col(c)||c.blue)+"40", borderRadius:"7px", padding:"10px 14px", marginBottom:"1rem", display:"flex", alignItems:"center", gap:"0.5rem" }}>
              <span style={{ color:TYPE_COLORS[form.type]?.col(c)||c.blue, fontSize:"0.75rem" }}>▣</span>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"1rem", flexWrap:"wrap" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"0.6rem", flexShrink:0 }}>
                  <button onClick={()=>move(idx,-1)} disabled={idx===0}
                    style={{ background:"none", border:"none", cursor:"pointer", color:c.muted, opacity:idx===0?0.3:1, fontSize:"0.9rem" }}>↑</button>
                  <button onClick={()=>move(idx,+1)} disabled={idx===banners.length-1}
                    style={{ background:"none", border:"none", cursor:"pointer", color:c.muted, opacity:idx===banners.length-1?0.3:1, fontSize:"0.9rem" }}>↓</button>
                  <Toggle on={banner.active} onChange={()=>toggleActive(banner)} c={c}/>
                  <button onClick={()=>startEdit(banner)}
                    style={{ background:"none", border:"1px solid "+c.border, borderRadius:"5px", cursor:"pointer", color:c.muted, fontSize:"0.74rem", padding:"3px 9px", fontFamily:gs }}>Edit</button>
                  <button onClick={()=>del(banner.id)}
                    style={{ background:"none", border:"none", cursor:"pointer", color:c.muted, fontSize:"0.8rem", padding:"2px 6px" }}>✕</button>
                </div>
              </div>
            </div>
          );
