...
const C = {
  dark:  { bg:"#090909", card:"#111113", surface:"#141416", border:"#232325", borderHi:"#333336", text:"#F0F0F0", muted:"#7A7A80", green:"#00E676", blue:"#4488FF", red:"#FF1800", redDim:"rgba(255,24,0,0.08)" },
  light: { bg:"#F7F7F5", card:"#FFFFFF",  surface:"#EEEEED", border:"#DEDEDD",  borderHi:"#BABAB8", text:"#0A0A0A", muted:"#606065", green:"#008A38", blue:"#1E55CC", red:"#CC0000", redDim:"rgba(204,0,0,0.08)" },
};
...
function Toggle({ on, onChange, c }) {
  return (
    <div onClick={onChange}
      style={{ width:"38px", height:"22px", borderRadius:"11px", background:on?c.green:c.surface, border:"1px solid "+(on?c.green:c.border), cursor:"pointer", position:"relative", transition:"opacity 0.22s ease", flexShrink:0 }}>
    ...
    <div style={{ padding:"2rem 2.5rem", maxWidth:"900px" }}>
      ...
      {showForm && (
        <form onSubmit={save} style={{ background:c.card, border:"1px solid "+c.borderHi, borderRadius:"14px", padding:"1.5rem", marginBottom:"1.5rem" }}>
          ...
          <div style={{ marginBottom:"0.85rem" }}>
            <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Banner Text *</label>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:"0.85rem", marginBottom:"1rem" }}>
            ...
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Starts</label>
              <input type="datetime-local" value={form.starts_at} onChange={e=>setForm(p=>({...p,starts_at:e.target.value}))} style={inp}/>
            </div>
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Ends</label>
          ...
          {form.text && (
            <div style={{ background:typeCol(form.type)+"1A", border:"1px solid "+typeCol(form.type)+"40", borderRadius:"8px", padding:"10px 14px", marginBottom:"1rem", display:"flex", alignItems:"center", gap:"0.5rem" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"0.85rem" }}>
        {loading ? Array.from({length:3}).map((_,i)=>(
          <div key={i} style={{ height:"80px", background:c.card, borderRadius:"14px", animation:"pulse 1.4s ease infinite" }}/>
        )) : banners.length === 0 ? (
          <div style={{ background:c.card, border:"1px solid "+c.border, borderRadius:"14px", padding:"3rem", textAlign:"center" }}>
          ...
          return (
            <div key={banner.id} style={{ background:c.card, border:"1px solid "+(banner.active&&!expired?tc+"40":c.border), borderRadius:"14px", padding:"1.1rem 1.25rem" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"1rem", flexWrap:"wrap" }}>
                <div style={{ flex:1, minWidth:"200px" }}>
                  <div style={{ display:"flex", gap:"0.4rem", flexWrap:"wrap", marginBottom:"0.5rem", alignItems:"center" }}>
                    ...
                    {scheduled && <span style={{ fontFamily:gs, fontSize:"0.65rem", color:c.muted, background:c.surface, borderRadius:"6px", padding:"2px 8px" }}>Scheduled</span>}
                  </div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"0.6rem", flexShrink:0 }}>
                  ...
                  <button onClick={()=>startEdit(banner)} style={{ background:"none", border:"1px solid "+c.border, borderRadius:"4px", cursor:"pointer", color:c.muted, fontSize:"0.74rem", padding:"3px 9px", fontFamily:gs }}>Edit</button>
