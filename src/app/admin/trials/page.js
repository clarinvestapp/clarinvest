...
const C = {
  dark:  { bg:"#090909", card:"#111113", surface:"#141416", border:"#232325", borderHi:"#333336", text:"#F0F0F0", muted:"#7A7A80", green:"#00E676", blue:"#4488FF", red:"#FF1800" },
  light: { bg:"#F7F7F5", card:"#FFFFFF",  surface:"#EEEEED", border:"#DEDEDD",  borderHi:"#BABAB8", text:"#0A0A0A", muted:"#606065", green:"#008A38", blue:"#1E55CC", red:"#CC0000" },
};
...
const PLAN_COLOR = (p, c) => p==="ultimate"?c.text:p==="pro"?c.green:c.muted;
    ...
    <div style={{ padding:"2rem 2.5rem", maxWidth:"900px" }}>
      ...
      {showForm && (
        <form onSubmit={save} style={{ background:c.card, border:`1px solid ${c.borderHi}`, borderRadius:"14px", padding:"1.5rem", marginBottom:"1.5rem" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:"0.85rem", marginBottom:"1rem" }}>
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Plan</label>
            ...
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Duration (days)</label>
              <input type="number" value={form.duration_days} onChange={e=>setForm(p=>({...p,duration_days:e.target.value}))} style={inp}/>
            </div>
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Max uses</label>
              <input type="number" value={form.max_uses} onChange={e=>setForm(p=>({...p,max_uses:e.target.value}))} style={inp}/>
            </div>
            <div>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Custom code (optional)</label>
            ...
            <div style={{ gridColumn:"1/-1" }}>
              <label style={{ display:"block", fontFamily:gs, fontSize:"0.65rem", color:c.muted, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:"5px", fontWeight:500 }}>Internal note</label>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
        {loading ? Array.from({length:3}).map((_,i)=>(
          <div key={i} style={{ height:"100px", background:c.card, borderRadius:"14px", animation:"pulse 1.4s ease infinite" }}/>
        )) : codes.length === 0 ? (
          ...
          return (
            <div key={code.id} style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:"14px", padding:"1.25rem 1.5rem" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:"0.75rem", marginBottom:"0.75rem" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"0.75rem", flexWrap:"wrap" }}>
                  {/* Code with copy */}
                  <button onClick={() => copyCode(code.code)}
                    title="Click to copy"
                    style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:"4px", padding:"5px 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:"6px", color:c.text }}>
                  ...
                  {/* Plan badge */}
                  <span style={{ fontFamily:gs, fontSize:"0.68rem", fontWeight:700, color:planCol, background:`${planCol}18`, border:`1px solid ${planCol}40`, borderRadius:"6px", padding:"3px 9px", textTransform:"capitalize" }}>
                    {code.plan}
                  </span>
                  {/* Duration */}
